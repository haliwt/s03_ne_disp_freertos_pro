#include "bsp.h"

#include "FreeRTOS.h"
#include "task.h"
#include "main.h"
#include "cmsis_os.h"


#define POWER_KEY_0	        (1 << 0)
#define MODE_KEY_1	        (1 << 1)
#define DEC_KEY_2           (1 << 2)
#define ADD_KEY_3           (1 << 3)



#define MODE_LONG_KEY_8         (1 << 8)



/***********************************************************************************************************
											鍑芥暟澹版槑
***********************************************************************************************************/
static void vTaskRunPro(void *pvParameters);
static void vTaskDecoderPro(void *pvParameters);
static void vTaskStart(void *pvParameters);
static void AppTaskCreate (void);



/* 鍒涘缓浠诲姟閫氫俊鏈哄埗 */
static void AppObjCreate(void);


/***********************************************************************************************************
											鍙橀噺澹版槑
***********************************************************************************************************/
static TaskHandle_t xHandleTaskRunPro = NULL;
static TaskHandle_t xHandleTaskDecoderPro= NULL;
static TaskHandle_t xHandleTaskStart = NULL;

//static QueueHandle_t xQueue1 = NULL;
static QueueHandle_t xQueue2 = NULL;
//static QueueHandle_t xQueue3 = NULL;



///#define QUEUE_LENGTH            1                   /* 闃熷垪鏀寔鐨勬秷鎭釜鏁� */
///#define QUEUE_ITEM_SIZE         sizeof(uint32_t)    /* 闃熷垪涓瘡鏉℃秷鎭殑澶у皬 */
//#define SEMAPHORE_BINARY_LENGTH 1                   /* 浜屽€间俊鍙烽噺鐨勬湁鏁堥暱搴� */
//#define QUEUESET_LENGTH         ((2 * QUEUE_LENGTH) + SEMAPHORE_BINARY_LENGTH)  /* 闃熷垪闆嗘敮鎸佺殑娑堟伅涓暟 */



typedef struct Msg
{
	uint8_t  ucMessageID;
	uint8_t usData[12];
	//uint8_t ulData[1];
}MSG_T;

MSG_T   g_tMsg; /* 瀹氫箟涓拷涓粨鏋勪綋鐢ㄤ簬娑堟伅闃熷垪 */

volatile uint8_t decoder_flag;

volatile uint8_t ulid,uldata,usdata;


uint32_t mode_key_long_conter;

uint8_t rxcmd[1];

volatile uint8_t rx_data_counter;

uint8_t rx_end_flag  ;
uint8_t rx_end_counter, rx_end_counter_compare;



/*
*********************************************************************************************************
*	鍑�1锟�7 鏁�1锟�7 鍚�1锟�7: vTaskTaskUserIF
*	鍔熻兘璇存槑: 鎺ュ彛娑堟伅澶勭悊銆�1锟�7
*	褰�1锟�7    鍙�1锟�7: pvParameters 鏄湪鍒涘缓璇ヤ换鍔℃椂浼狅拷锟界殑褰㈠弬
*	杩�1锟�7 鍥�1锟�7 鍊�1锟�7: 鏃�1锟�7
*   浼�1锟�7 鍏�1锟�7 绾�1锟�7: 1  (鏁帮拷锟借秺灏忎紭鍏堢骇瓒婁綆锛岃繖涓窡uCOS鐩稿弽)
*********************************************************************************************************
*/
void freeRTOS_Handler(void)
{
     /* 鍒涘缓浠诲姟 */
	  AppTaskCreate();
	  
	  /* 鍒涘缓浠诲姟閫氫俊鏈哄埗 */
	   AppObjCreate();
	  
	  /* 鍚姩璋冨害锛屽紑濮嬫墽琛屼换鍔�1锟�7 */
	   vTaskStartScheduler();


}
/**********************************************************************************************************
*   FunctionName: static void vTaskRunPro(void *pvParameters)
*	鍔熻兘璇存槑: 浣跨敤鍑芥暟xTaskNotifyWait鎺ユ敹浠诲姟vTaskTaskUserIF鍙戦€佺殑浜嬩欢鏍囧織浣嶈缃�
*	褰�    鍙�: pvParameters 鏄湪鍒涘缓璇ヤ换鍔℃椂浼犻€掔殑褰㈠弬
*	杩� 鍥� 鍊�: 鏃�
*   浼� 鍏� 绾�: 1  

*   浼�1锟�7 鍏�1锟�7 绾�1锟�7: 1  (鏁帮拷锟借秺灏忎紭鍏堢骇瓒婁綆锛岃繖涓窡uCOS鐩稿弽)
**********************************************************************************************************/
static void vTaskRunPro(void *pvParameters)
{
    BaseType_t xResult;
	const TickType_t xMaxBlockTime = pdMS_TO_TICKS(10); /* 璁剧疆鏈€澶х瓑寰呮椂闂翠负30ms */
	uint32_t ulValue;
    
    static volatile uint8_t power_on_off_flag,fan_on_off_flag ;
    static uint8_t dry_on_off_flag,plasma_on_off_flag, ai_on_off_flag ;
    static uint8_t key_add_flag,key_dec_flag,key_mode_flag;
    
    while(1)
    {
		/*
			绗竴涓弬鏁� ulBitsToClearOnEntry鐨勪綔鐢紙鍑芥暟鎵ц鍓嶏級锛�
		          ulNotifiedValue &= ~ulBitsToClearOnEntry
		          绠€鍗曠殑璇村氨鏄弬鏁皍lBitsToClearOnEntry閭ｄ釜浣嶆槸1锛岄偅涔坣otification value
		          鐨勯偅涓綅灏变細琚竻闆躲€�

		          杩欓噷ulBitsToClearOnEntry = 0x00000000灏辨槸鍑芥暟鎵ц鍓嶄繚鐣欐墍鏈変綅銆�
		
		    绗簩涓弬鏁� ulBitsToClearOnExit鐨勪綔鐢紙鍑芥暟閫€鍑哄墠锛夛細			
				  ulNotifiedValue &= ~ulBitsToClearOnExit
		          绠€鍗曠殑璇村氨鏄弬鏁皍lBitsToClearOnEntry閭ｄ釜浣嶆槸1锛岄偅涔坣otification value
		          鐨勯偅涓綅灏变細琚竻闆躲€�

				  杩欓噷ulBitsToClearOnExi = 0xFFFFFFFF灏辨槸鍑芥暟閫€鍑哄墠娓呮鎵€鏈変綅銆�
		
		    娉細ulNotifiedValue琛ㄧず浠诲姟vTaskMsgPro鐨勪换鍔℃帶鍒跺潡閲岄潰鐨勫彉閲忋€�		
		*/
		
		xResult = xTaskNotifyWait(0x00000000,      
						          0xFFFFFFFF,      
						          &ulValue,        /* 淇濆瓨ulNotifiedValue鍒板彉閲弖lValue涓� */
						          xMaxBlockTime);  /* 鏈€澶у厑璁稿欢杩熸椂闂� */
		
		if( xResult == pdPASS )
		{
			/* 鎺ユ敹鍒版秷鎭紝妫€娴嬮偅涓綅琚寜涓� */
             
			if((ulValue & POWER_KEY_0) != 0)
			{
        	  power_on_off_flag = 1;
              
            }
            else if((ulValue & MODE_KEY_1) != 0){
            if(run_t.ptc_warning ==0 && run_t.fan_warning ==0){
                if(run_t.gPower_On == power_on)
                 key_mode_flag = 1;
              
               }

            }
            else if((ulValue & DEC_KEY_2) != 0){
                if(run_t.gPower_On == power_on)
                  key_dec_flag = 1;
              

            }
            else if((ulValue & ADD_KEY_3) != 0){
                if(run_t.gPower_On == power_on)
                  key_add_flag = 1;
              
           
            }
          
          

        
    }
    else{

     
        if(power_on_off_flag == 1){
             power_on_off_flag++;
             if(run_t.gPower_On == power_off){
                power_key_short_fun();

              }
              else{

                 run_t.gPower_On = power_off;

              }

            }
            else if(key_add_flag ==1 || key_dec_flag ==1 || key_mode_flag == 1){
                

              if(key_add_flag == 1){

                  key_add_flag ++;
                  add_key_fun();

              }
              else if(key_dec_flag == 1){
                 key_dec_flag ++;
                 dec_key_fun();

              } 
              else if(key_mode_flag == 1){
                 key_mode_flag++;
                 SendData_Buzzer();
                 mode_key_short_fun();

                }
        }
          
          
      if(run_t.gPower_On == power_on){

       disp_temp_humidity_wifi_icon_handler();
      
       disp_numbers_five_eight_and_fan_icon_handler();

       disp_time_colon_ion_handler();

       Timing_Handler();
       
       }
       else if(run_t.gPower_On == power_off){

          power_off_handler();

       }
     // USART1_Cmd_Error_Handler();

    }

   }
}
/**********************************************************************************************************
*	鍑�1锟�7 鏁�1锟�7 鍚�1锟�7: vTaskStart
*	鍔熻兘璇存槑: 鍚姩浠诲姟锛屼篃灏辨槸鏈拷楂樹紭鍏堢骇浠诲姟锛岃繖閲岀敤浣滄寜閿壂鎻忥拷锟�1锟�7
*	褰�1锟�7    鍙�1锟�7: pvParameters 鏄湪鍒涘缓璇ヤ换鍔℃椂浼狅拷锟界殑褰㈠弬
*	杩�1锟�7 鍥�1锟�7 鍊�1锟�7: 鏃�1锟�7
*   浼�1锟�7 鍏�1锟�7 绾�1锟�7: 4  (鏁帮拷锟借秺灏忎紭鍏堢骇瓒婁綆锛岃繖涓窡uCOS鐩稿弽)
**********************************************************************************************************/
static void vTaskDecoderPro(void *pvParameters)
{
    MSG_T *ptMsg;
	BaseType_t xResult;
	const TickType_t xMaxBlockTime = pdMS_TO_TICKS(300); /* 璁剧疆鏈€澶х瓑寰呮椂闂翠负30ms */
	//uint8_t uldata,usdata;
	
    while(1)
    {
		
       xResult = xQueueReceive(xQueue2,                   /* 娑堟伅闃熷垪鍙ユ焺 */
		                        (void *)&ptMsg,  		   /* 杩欓噷鑾峰彇鐨勬槸缁撴瀯浣撶殑鍦板潃 */
		                        (TickType_t)xMaxBlockTime);/* 璁剧疆闃诲鏃堕棿 */
		
		if(xResult == pdPASS){
            
          ulid = ptMsg ->ucMessageID;

        //  usdata = ptMsg->usData[0];

         // uldata = ptMsg->ulData[0];
          
           receive_data_fromm_mainboard(ptMsg->usData,ulid);
         }
      
    }
}

/**********************************************************************************************************
*	鍑�1锟�7 鏁�1锟�7 鍚�1锟�7: vTaskStart
*	鍔熻兘璇存槑: 娑堟伅澶勭悊锛屼娇鐢ㄥ嚱鏁癱omGetChar鑾峰彇涓插彛鍛戒护锛屼娇鐢ㄥ嚱鏁癱omSendBuf鍙戯拷锟戒覆鍙ｆ秷鎭�1锟�7
*	褰�1锟�7    鍙�1锟�7: pvParameters 鏄湪鍒涘缓璇ヤ换鍔℃椂浼狅拷锟界殑褰㈠弬
*	杩�1锟�7 鍥�1锟�7 鍊�1锟�7: 鏃�1锟�7
*   浼�1锟�7 鍏�1锟�7 绾�1锟�7: 3  (鏁帮拷锟借秺灏忎紭鍏堢骇瓒婁綆锛岃繖涓窡uCOS鐩稿弽)
**********************************************************************************************************/
static void vTaskStart(void *pvParameters)
{
	

    while(1)
    {
      
    if(KEY_POWER_GetValue()  ==KEY_DOWN){

           
           xTaskNotify(xHandleTaskRunPro, /* 鐩爣浠诲姟 */
                        POWER_KEY_0,            /* 璁剧疆鐩爣浠诲姟浜嬩欢鏍囧織浣峛it0  */
                         eSetBits);          /* 灏嗙洰鏍囦换鍔＄殑浜嬩欢鏍囧織浣嶄笌BIT_0杩涜鎴栨搷浣滐紝  灏嗙粨鏋滆祴鍊肩粰浜嬩欢鏍囧織浣嶃€�*/

             

     }
     else if(KEY_MODE_GetValue() ==KEY_DOWN){

        
        // while(MODEL_KEY_VALUE() == KEY_DOWN && mode_key_long_conter < 2965500){

//               mode_key_long_conter++;
//               if(mode_key_long_conter > 2965000){
//                   mode_key_long_conter = 2965900;
//               
//               xTaskNotify(xHandleTaskRunPro, /* 鐩爣浠诲姟 */
//                         MODE_LONG_KEY_8,            /* 璁剧疆鐩爣浠诲姟浜嬩欢鏍囧織浣峛it0  */
//                         eSetBits);          /* 灏嗙洰鏍囦换鍔＄殑浜嬩欢鏍囧織浣嶄笌BIT_0杩涜鎴栨搷浣滐紝  灏嗙粨鏋滆祴鍊肩粰浜嬩欢鏍囧織浣嶃€�*/
//
//                }
//
//
//         }
         
         //if(mode_key_long_conter < 2965000 ){
            
           xTaskNotify(xHandleTaskRunPro, /* 鐩爣浠诲姟 */
                         MODE_KEY_1,            /* 璁剧疆鐩爣浠诲姟浜嬩欢鏍囧織浣峛it0  */
                         eSetBits);          /* 灏嗙洰鏍囦换鍔＄殑浜嬩欢鏍囧織浣嶄笌BIT_0杩涜鎴栨搷浣滐紝  灏嗙粨鏋滆祴鍊肩粰浜嬩欢鏍囧織浣嶃€�*/

         // }


     }
     else if(KEY_DEC_GetValue() == KEY_DOWN){

           xTaskNotify(xHandleTaskRunPro, /* 鐩爣浠诲姟 */
                            DEC_KEY_2,            /* 璁剧疆鐩爣浠诲姟浜嬩欢鏍囧織浣峛it0  */
                            eSetBits);          /* 灏嗙洰鏍囦换鍔＄殑浜嬩欢鏍囧織浣嶄笌BIT_0杩涜鎴栨搷浣滐紝  灏嗙粨鏋滆祴鍊肩粰浜嬩欢鏍囧織浣嶃€�*/

     }
     else if(KEY_ADD_GetValue() ==KEY_DOWN){

          xTaskNotify(xHandleTaskRunPro, /* 鐩爣浠诲姟 */
                            ADD_KEY_3,            /* 璁剧疆鐩爣浠诲姟浜嬩欢鏍囧織浣峛it0  */
                            eSetBits);          /* 灏嗙洰鏍囦换鍔＄殑浜嬩欢鏍囧織浣嶄笌BIT_0杩涜鎴栨搷浣滐紝  灏嗙粨鏋滆祴鍊肩粰浜嬩欢鏍囧織浣嶃€�*/

    }
    

    

     vTaskDelay(20);
     
    }

}
/**********************************************************************************************************
*	鍑�1锟�7 鏁�1锟�7 鍚�1锟�7: AppTaskCreate
*	鍔熻兘璇存槑: 鍒涘缓搴旂敤浠诲姟
*	褰�1锟�7    鍙傦細鏃�1锟�7
*	杩�1锟�7 鍥�1锟�7 鍊�1锟�7: 鏃�1锟�7
**********************************************************************************************************/
void AppTaskCreate (void)
{

	xTaskCreate( vTaskRunPro,    		/* 浠诲姟鍑芥暟  */
                 "vTaskRunPro",  		/* 浠诲姟鍚�1锟�7    */
                 128,         		/* stack澶у皬锛屽崟浣峸ord锛屼篃灏辨槸4瀛楄妭 */
                 NULL,        		/* 浠诲姟鍙傛暟  */
                 1,           		/* 浠诲姟浼樺厛绾�1锟�7 鏁帮拷锟借秺灏忎紭鍏堢骇瓒婁綆锛岃繖涓窡uCOS鐩稿弽 */
                 &xHandleTaskRunPro); /* 浠诲姟鍙ユ焺  */
	
	xTaskCreate( vTaskDecoderPro,     		/* 浠诲姟鍑芥暟  */
                 "vTaskDecoderPro",   		/* 浠诲姟鍚�1锟�7    */
                 128,             		/* 浠诲姟鏍堝ぇ灏忥紝鍗曚綅word锛屼篃灏辨槸4瀛楄妭 */
                 NULL,           		/* 浠诲姟鍙傛暟  */
                 2,               		/* 浠诲姟浼樺厛绾�1锟�7 鏁帮拷锟借秺灏忎紭鍏堢骇瓒婁綆锛岃繖涓窡uCOS鐩稿弽 */
                 &xHandleTaskDecoderPro );  /* 浠诲姟鍙ユ焺  */
	
	
	xTaskCreate( vTaskStart,     		/* 浠诲姟鍑芥暟  */
                 "vTaskStart",   		/* 浠诲姟鍚�1锟�7    */
                 128,            		/* 浠诲姟鏍堝ぇ灏忥紝鍗曚綅word锛屼篃灏辨槸4瀛楄妭 */
                 NULL,           		/* 浠诲姟鍙傛暟  */
                 3,              		/* 浠诲姟浼樺厛绾�1锟�7 鏁帮拷锟借秺灏忎紭鍏堢骇瓒婁綆锛岃繖涓窡uCOS鐩稿弽 */
                 &xHandleTaskStart );   /* 浠诲姟鍙ユ焺  */
}



/*
*********************************************************************************************************
*	鍑�1锟�7 鏁�1锟�7 鍚�1锟�7: AppObjCreate
*	鍔熻兘璇存槑: 鍒涘缓浠诲姟閫氫俊鏈哄埗
*	褰�1锟�7    鍙�1锟�7: 鏃�1锟�7
*	杩�1锟�7 鍥�1锟�7 鍊�1锟�7: 鏃�1锟�7
*********************************************************************************************************
*/
# if 1 
void AppObjCreate (void)
{
    #if 1

//   /* 鍒涘缓10涓猽int8_t鍨嬫秷鎭槦鍒�1锟�7 */
//	xQueue1 = xQueueCreate(4, sizeof(uint8_t));
//    if( xQueue1 == 0 )
//    {
//        /* 娌℃湁鍒涘缓鎴愬姛锛岀敤鎴峰彲浠ュ湪杩欓噷鍔犲叆鍒涘缓澶辫触鐨勫鐞嗘満鍒�1锟�7 */
//    }
	
	/* 鍒涘缓10涓瓨鍌ㄦ寚閽堝彉閲忕殑娑堟伅闃熷垪锛岀敱浜嶤M3/CM4鍐呮牳鏄�1锟�732浣嶆満锛屼竴涓寚閽堝彉閲忓崰鐢�1锟�74涓瓧鑺�1锟�7 */
	xQueue2 = xQueueCreate(10, sizeof(struct Msg *));
    if( xQueue2 == 0 )
    {
        /* 娌℃湁鍒涘缓鎴愬姛锛岀敤鎴峰彲浠ュ湪杩欓噷鍔犲叆鍒涘缓澶辫触鐨勫鐞嗘満鍒�1锟�7 */
    }

	

	#endif 

    #if 0

	 /* 鍒涘缓闃熷垪闆�1锟�7 */
    xQueueSet = xQueueCreateSet(QUEUESET_LENGTH);
    /* 鍒涘缓闃熷垪*/
    xQueue1 = xQueueCreate(QUEUE_LENGTH, QUEUE_ITEM_SIZE);
    xQueue2 = xQueueCreate(QUEUE_LENGTH, QUEUE_ITEM_SIZE);
	
    /* 鍒涘缓浜岋拷锟戒俊鍙烽噺 */
    xSemaphore = xSemaphoreCreateBinary();
	
    /* 灏嗛槦鍒楀拰浜岋拷锟戒俊鍙烽噺娣诲姞鍒伴槦鍒楅泦涓�1锟�7 */
    xQueueAddToSet(xQueue1, xQueueSet);
    xQueueAddToSet(xQueue2, xQueueSet);
    xQueueAddToSet(xSemaphore, xQueueSet);
    #endif 
}

#endif 
/********************************************************************************
	**
	*Function Name:void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
	*Function :UART callback function  for UART interrupt for receive data
	*Input Ref: structure UART_HandleTypeDef pointer
	*Return Ref:NO
	*
*******************************************************************************/
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
     static uint8_t state, rx_mb_data_tag;
     BaseType_t xHigherPriorityTaskWoken = pdFALSE;
     MSG_T *ptMsg;


    if(huart==&huart1) // Motor Board receive data (filter)
	{

       DISABLE_INT();
       switch(state)
		{
		case 0:  //#0
			if(inputBuf[0]==0x5A)  {//hex :4D - "M" -fixed mainboard
               
             
				state=1; 
		    }
			else{
			    state=0; //=1
              

			}
			break;
		case 1: //#1
		       
                   ptMsg = &g_tMsg;
                   ptMsg->usData[rx_data_counter] =  inputBuf[0];
                   rx_data_counter++;

                   if(inputBuf[rx_data_counter]==0x0A && rx_end_flag == 0){
                     
                          rx_end_flag = 1 ;
                          rx_end_counter = rx_data_counter;
                          
                    }
                    else if(rx_end_flag == 1){

                      rx_end_counter_compare = rx_data_counter - rx_end_counter;
                      if(rx_end_counter_compare == 1){
                   
                          if(inputBuf[rx_data_counter]==0x0D){

                              rx_end_counter_compare ++ ;


                          }

                        }
                        else{
                           rx_end_flag = 0;
                         


                        }


                    }
                    
                   

               
               if(rx_end_counter_compare == 2){

                      rx_end_counter_compare=0;
                      rx_end_flag=0;
                      rx_data_counter=0;
                   /* 鍚戞秷鎭槦鍒楀彂鏁版嵁 */
                     /* 鍒濆鍖栫粨鏋勪綋鎸囬拡 */
                     ptMsg = &g_tMsg;
                     ptMsg->ucMessageID=rx_data_counter;
                     
                	xQueueSendFromISR(xQueue2,
                				      (void *)&ptMsg,
                				      &xHigherPriorityTaskWoken);

                	/* 濡傛灉xHigherPriorityTaskWoken = pdTRUE锛岄偅涔堥€€鍑轰腑鏂悗鍒囧埌褰撳墠鏈€楂樹紭鍏堢骇浠诲姟鎵ц */
                	portYIELD_FROM_ISR(xHigherPriorityTaskWoken);
			

               }
		      
			break;
       #if 0
		case 2://#2
			if(inputBuf[0]=='D'||inputBuf[0]==0x4B || inputBuf[0]=='E'){
				if(inputBuf[0]=='D'){
                    rx_mb_data_tag=PANEL_DATA;
                    state = 3;

                }
                else if(inputBuf[0]=='E'){
                  rx_mb_data_tag=ORDER_DATA;
                  state=3;
                }
                else if(inputBuf[0]==0x4B){
                    state = 5;

                }
				
			}
            else{
				state=0;
				


				}
			break;
            
        case 3:

            if(rx_mb_data_tag==PANEL_DATA){
            
                // run_t.gReal_humtemp[0]=inputBuf[0]; //Humidity value 
                  rxcmd[0]=inputBuf[0];
                  state = 4; 
                
             }
			else if(rx_mb_data_tag==ORDER_DATA){

                rxcmd[0]=  inputBuf[0];  
                /* 鍒濆鍖栫粨鏋勪綋鎸囬拡 */
                ptMsg = &g_tMsg;
                ptMsg->ucMessageID=ORDER_DATA;
    		    ptMsg->ulData[0] = inputBuf[0]; //rxcmd[0];
                ptMsg ->usData[0] = 0;
               
               

                 /* 鍚戞秷鎭槦鍒楀彂鏁版嵁 */
            	xQueueSendFromISR(xQueue2,
            				      (void *)&ptMsg,
            				      &xHigherPriorityTaskWoken);

            	/* 濡傛灉xHigherPriorityTaskWoken = pdTRUE锛岄偅涔堥€€鍑轰腑鏂悗鍒囧埌褰撳墠鏈€楂樹紭鍏堢骇浠诲姟鎵ц */
            	portYIELD_FROM_ISR(xHigherPriorityTaskWoken);

             
           

                state=0;

			}
            else {
               state = 0; 
          
            }
	    break;
        
		case 4: //

		 if(rx_mb_data_tag==PANEL_DATA){
              //run_t.gReal_humtemp[1]=inputBuf[0]; //temperature value
              
               /* 鍒濆鍖栫粨鏋勪綋鎸囬拡 */
                ptMsg = &g_tMsg;
                ptMsg->ucMessageID = PANEL_DATA;
    		    ptMsg->ulData[0] =  rxcmd[0];
                ptMsg ->usData[0] = inputBuf[0];
               
               

                 /* 鍚戞秷鎭槦鍒楀彂鏁版嵁 */
            	xQueueSendFromISR(xQueue2,
            				      (void *)&ptMsg,
            				      &xHigherPriorityTaskWoken);

            	/* 濡傛灉xHigherPriorityTaskWoken = pdTRUE锛岄偅涔堥€€鍑轰腑鏂悗鍒囧埌褰撳墠鏈€楂樹紭鍏堢骇浠诲姟鎵ц */
            	portYIELD_FROM_ISR(xHigherPriorityTaskWoken);
			
		     state=0;
		    
		
              
          }
         else {

		
			state =0;

         }
		
		 
		 break;

         case 5:

           rxcmd[0]=inputBuf[0];

           state = 6;


         break;


		 case 6:

            /* 鍒濆鍖栫粨鏋勪綋鎸囬拡 */
            ptMsg = &g_tMsg;
            ptMsg->ucMessageID=ANSWER_DATA;
		    ptMsg->ulData[0] = rxcmd[0];
            ptMsg ->usData[0] =  inputBuf[0];
           
           

             /* 鍚戞秷鎭槦鍒楀彂鏁版嵁 */
        	xQueueSendFromISR(xQueue2,
        				      (void *)&ptMsg,
        				      &xHigherPriorityTaskWoken);

        	/* 濡傛灉xHigherPriorityTaskWoken = pdTRUE锛岄偅涔堥€€鍑轰腑鏂悗鍒囧埌褰撳墠鏈€楂樹紭鍏堢骇浠诲姟鎵ц */
        	portYIELD_FROM_ISR(xHigherPriorityTaskWoken);
	       
		 
             state=0;
        break;
           
       
		
		
		default:
			
		break;
		}
      #endif 

          ENABLE_INT();
       __HAL_UART_CLEAR_OREFLAG(&huart1);
		HAL_UART_Receive_IT(&huart1,inputBuf,1);//UART receive data interrupt 1 byte
	}
}
}
void USART1_Cmd_Error_Handler(void)
{
   uint32_t temp;
   if(run_t.gTimer_usart_error >4){
	  	run_t.gTimer_usart_error=0;
	    
         
        __HAL_UART_CLEAR_OREFLAG(&huart1);
        temp = USART1->RDR;
		UART_Start_Receive_IT(&huart1,inputBuf,1);
       
		}
      
    
  
 }

















